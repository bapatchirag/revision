...
- **Scope the session smaller.** `svn` is always run from the source directory, so
  <KeyCap key="P" /> — [change source path](/reference/cli/#--path) — narrows status and
  diff to one subtree instead of the whole checkout.
- **Act on fewer files at once.** A directory-wide stage, revert or delete is a single
  `svn` invocation under a single deadline; the same files one at a time each get their
  own.
