...
install.sh: no prebuilt binary for linux-arm64. Build from source instead:
    go install github.com/bapatchirag/revision/cmd/revision@latest
```

Prebuilt binaries exist for `darwin-arm64` and `linux-amd64` only. Everywhere else,
build from source:
...
That covers Intel Macs, ARM Linux, and anything else Go targets. Note that builds produced
this way never [self-update](/operations/updating-revision/) — re-run `go install` to
upgrade.
